...
### 3.2. Set initial Password for Business users
  1. In the **User Management** screen, search for and click on a user from the "bestrun" domain (e.g., alice.jones@bestrun.com).
...
5. **User Name, E-mail:** Enter the email address "alice.jones@bestrun.com" in both the User Name and E-mail address fields. It must must also match the email used in the IAS import.
...
### 5.2.  Set Up and Import the Secure Incident Management Project

1. **Open the workspace folder:** Navigate to the **Hamburger Menu (☰) → File → Open Folder...**
2. **Select the target directory:** In the file selection dialog, locate and select the **projects** folder, then click OK.
3. Open a **new terminal:** Navigate to the **Hamburger Menu (☰) → Terminal → New Terminal**.
...
5. Copy and execute the following command in your terminal to download, extract, and automatically navigate into the Secure Incident Management application folder:
   ...
   ```
   mkdir -p secure_incident_management && cd secure_incident_management && curl -L https://github.com/SAP-samples/btp-secure-development/releases/download/v1.0.1/secure_incident_management.zip -o secure_incident_management.zip && unzip -q secure_incident_management.zip
   ```
...
#### 6.6.1. Check and Start SAP HANA Database
  1. In the subaccount cockpit, in the left navigation menu, go to **Instances and Subscriptions** → **Applications**.
