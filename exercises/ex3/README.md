...
<details>
<summary>Click to expand hidden note</summary>

⚠️ Note: Exercise 3.2 requires a production landscape because writing custom audit log events from applications is only supported with the paid **SAP Audit Log, Premium or Standard service plan**. This paid plan is not included in trial accounts.
  Trial accounts do include the **SAP Audit Log Management service default plan** at no cost. This service provides access to audit logs generated by SAP services and applications on the user's behalf. These system-generated logs remain available for retrieval at all times through the audit log viewer if the plan is subscribed to. See [SAP Audit Log Service](https://discovery-center.cloud.sap/protected/index.html#/serviceCatalog/audit-log-service?region=all)
