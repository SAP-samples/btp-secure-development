...
Exercise 3.2 requires a production landscape because writing custom audit log events from applications is only supported with the paid "Audit Log, Premium/Standard" service plan. This paid plan is not included in trial accounts.
Trial accounts do include the "Audit Log Management" service plan at no cost. This service provides access to audit logs generated by SAP services and applications on the user's behalf. These system-generated logs remain available for retrieval at all times through the audit log viewer if the plan is subscribed to.
